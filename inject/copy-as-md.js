...
function normalizeText(text) {
  return text.replace(/(^"|"$)/g, '').replace(/(\n|\n\r)/g, '<br />').replace(/<\/p>\s*##/g, '</p><br/>##');
}

function copy() {
  console.log('copy as md',);
  ...
  if (wrapper) {
    ...
    while (i < len) {
      ...
      text = elem.querySelector(`${SELECTOR_NAME.noteContent} ${SELECTOR_NAME.noteText}`);
      title && title.innerHTML && mdList.push(`<h2>${title.innerHTML}</h2>`);
      text && text.innerHTML && mdList.push(`<p>${(text.innerHTML||'').replace(/\n/g, '<br />')}</p>`);
      i++;
    }
